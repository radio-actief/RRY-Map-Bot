...
const columns = {
  ...
  link: {
    label: "Meshcore link",
    value: (val) =>
      `<a href="javascript:navigator.clipboard.writeText('${val}')">Copy to clipboard</a>`,
  },
};
        ...
        node[key]
          ? [
              `<td><b>${columns[key].label}</b></td><td>${
                columns[key].value ? columns[key].value(node[key]) : node[key]
              }</td>`,
            ]
          : []
      )
...
createApp({
  setup() {
    ...
    watch([() => app.nodeFilter, () => app.fromDate], () => {
    ...
    const stats = computed(() => {
      ...
      result.push(`
				...
				<i class="node-type pointer-help" title="Total client nodes">person</i><b>${
          nodes.filter((n) => n.type === 1).length
        }</b>&nbsp;|
				<i class="node-type pointer-help" title="Total repeater nodes">cell_tower</i><b>${
          nodes.filter((n) => n.type === 2).length
        }</b>&nbsp;|
				<i class="node-type pointer-help" title="Total room server nodes">forum</i><b>${
          nodes.filter((n) => n.type === 3).length
        }</b>
			`);
      result.push(
        `<span class="pointer-help" title="Nodes added in last 24 hours">24h: <b>${
          app.nodes.filter((n) => isNewerThan(n.inserted_date, 1)).length
        }</b></span>`
      );
      result.push(
        `<span class="pointer-help" title="Nodes added in last 7 days">7d: <b>${
          app.nodes.filter((n) => isNewerThan(n.inserted_date, 7)).length
        }</b></span>`
      );
      result.push(
        `<span class="pointer-help" title="Nodes added in last 30 days">30d: <b>${
          app.nodes.filter((n) => isNewerThan(n.inserted_date, 30)).length
        }</b></span>`
      );
    ...
    const searchResults = computed(() => {
      ...
      return app.filteredNodes
        .filter(
          (node) =>
            node.adv_name.toLowerCase().includes(app.search.toLowerCase()) ||
            node.public_key.startsWith(app.search)
        )
    ...
    onMounted(() => {
      ...
      if (location.hash === "#add-new-node") {
        ...
        dialogAddNode.value.addEventListener("close", () =>
          clearLocationHash()
        );
      }
    });
